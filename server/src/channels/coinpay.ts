/**
 * CoinPay SDK 模块（重构版 v2）
 * 对接文档：https://docs.coinpay.io/  (Gitee: https://gitee.com/coinpay/docs)
 *
 * 核心差异（vs 旧版）：
 * - API 地址：https://pay.coinpay.io/gateway/pay/checkout（form-data，非 JSON）
 * - 签名：sha256(secretKey + requestId + merchantNo + orderNo + orderAmount + orderCurrency)
 * - 返回：nextStep=redirect 时，跳转至 CoinPay 收银台（用户扫码/复制地址充值）
 * - 状态同步：CoinPay 通过 IPN 回调 notifyURL 通知商户
 */
...
export interface CoinPayOrderResult {
  reference: string;      // CoinPay 平台订单号（CWSxxx）
...
export class CoinPaySDK {
  ...
  constructor(config: CoinPayConfig) {
    ...
    this.apiBaseUrl = config.apiBaseUrl ? config.apiBaseUrl : 'https://pay.coinpay.io';
  }
   ...
   */
  async createOrder(params: CreateOrderParams): Promise<CoinPayOrderResult> {
    ...
    // 3. 调用 CoinPay API（form-data POST）
    ...
    try {
      ...
      throw new Error('[CoinPay] 网络请求失败: ' + err.message);
    }

    const data: any = await resp.json();

    if (data.respCode !== 200) {
      throw new Error('[CoinPay] respCode=' + data.respCode + ' respMessage=' + (data.respMessage || 'Unknown'));
    }
